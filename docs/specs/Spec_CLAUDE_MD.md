# Spec: CLAUDE.md — Project Constitution

> **Component:** `.claude/CLAUDE.md` (Claude Code reads this on every session start)
> **Priority:** P0 — Claude Code's behavior is defined by this file
> **Constraint:** ≤100 lines. Every line must earn its place.

## 1. Objective

Create the `CLAUDE.md` file that acts as the project constitution. Claude Code reads this
at the start of every session. It defines WHO Claude is in this project, HOW it operates,
and WHAT conventions it must follow. This is NOT documentation — it's behavioral programming.

## 2. Design Principles

- **Concise:** ≤100 lines. Claude's context window is precious. No filler.
- **Behavioral:** Every line should change how Claude acts, not just inform.
- **Convention-heavy:** File naming, YAML frontmatter schemas, folder semantics.
- **Safety-first:** DRY_RUN defaults, HITL requirements, never-do rules.

## 3. Required Sections

### 3A. Identity Block (5–8 lines)
...
### 3B. Folder Semantics (10–12 lines)

Define what each folder means and the flow between them:
...
### 3C. File Conventions (15–20 lines)

Define the YAML frontmatter schema that ALL action files must follow:
...
### 3G. Coding Conventions (8–10 lines)
...
- [ ] File is at `.claude/CLAUDE.md` (NOT project root)
- [ ] Total line count ≤100 (including blank lines)
- [ ] Every section heading starts with `##`
- [ ] YAML frontmatter schema is copy-pasteable
- [ ] Log JSON schema is valid JSON
- [ ] Safety rules are numbered 1–10
- [ ] No TODO placeholders or TBD notes — everything is concrete
- [ ] Running `wc -l .claude/CLAUDE.md` returns ≤100

## 5. Anti-Patterns to Avoid

- Do NOT include implementation details of watchers or MCP servers
- Do NOT include full API documentation
- Do NOT duplicate what's in spec files
- Do NOT include motivational text or comments like "you're a great assistant"
- Do NOT include version numbers of dependencies (those live in pyproject.toml)
